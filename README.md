...
- BFF consumes `adgroup-events` into its read model and serves them via `GET /campaigns` (adGroups array). On restart it resets to earliest offsets for its stable consumer groups to rebuild in-memory state.
